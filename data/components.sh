...
#
# Input: owner/repo per line on stdin.
...
while read -r r; do
  ...
  [ -s "$f" ] || { printf '%s\t\tno tree\tDeferred\t%s\n' "$r" "$DETECTOR_VERSION"; continue; }
  ...
  done < <(grep -iE '\.kicad_pcb$' "$f" | head -4)
  ...
  if [ -z "$src" ]; then
    ...
    done < <(grep -iE '(^|/)[^/]*bom[^/]*\.(csv|md|txt|tsv)$' "$f" | head -2)
  fi

  if [ -z "$src" ]; then
    printf '%s\t\tno .kicad_pcb and no machine-readable BOM\tDeferred\t%s\n' "$r" "$DETECTOR_VERSION"; continue
  fi
  ...
  printf '%s\t%s\t%s: smd=%s tht=%s (panel excluded) smd_ic=%s\t%s\t%s\n' \
    "$r" "$v" "$src" "$smd" "$tht" "$ic" "$conf" "$DETECTOR_VERSION"
done
